...
static bool _is_pfail(const struct device *dev)
{
    ...
    err = w25m02gw_spi_transceive(dev, tx_buffer, 2, rx_buffer, 3);
    uint8_t status3 = rx_buffer[2];
    printk("Status3: %x", status3);

    return (status3 & BIT(3));
}
static bool check_fail(const struct device *dev)
{
    int err = 0;
    uint8_t tx_buffer[2];
    uint8_t rx_buffer[3];
    ...
    if(status3 & BIT(3))
    {
        printk("Warning PFail bit is set");

    }
    if(status3 & BIT(2))
    {
        printk("Warning EFail bit is set");
    }
...
static int _load_data_to_write(const struct device *dev, uint16_t column_addr, uint8_t * buffer, size_t buffer_size)
{
    ...
    err = w25m02gw_spi_transceive(dev, spi_tx_data, buffer_size + 3, NULL, 0);
...
static int init_gpios(const struct device *dev)
{
    ...
    err = gpio_pin_configure_dt(&cfg->wp, GPIO_OUTPUT_INACTIVE);
    if (err) {
...
static int init_w25m02gw(const struct device *dev)
{
    ...
    if (err < 0) {
        printk("Error reading the first page of block 0");
        return err;
    }
